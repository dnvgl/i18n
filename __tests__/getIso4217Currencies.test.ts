...
describe('getIso4217Currencies', () => {
  ...
  test('Bulgarian Lev (BGN) currency is returned', () => {
    const countries = getIso4217Currencies("2025-12-31");
    expect(countries.find(x => x.alpha3Code === "BGN")).toBeDefined();
  });

  test('Bulgarian Lev (BGN) currency is not returned', () => {
    const countries = getIso4217Currencies("2026-01-01");
    expect(countries.find(x => x.alpha3Code === "BGN")).toBeUndefined();
  });
